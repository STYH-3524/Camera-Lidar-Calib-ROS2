                        ...
                        const std::string &axis_mode = "camera")
{
  ...
  pcl::computeCovarianceMatrixNormalized(*pc, centroid, covariance_matrix);
  ...
  Eigen::Vector3f axis_V = solver.eigenvectors().col(1).normalized(); 

  std::vector<std::pair<float, int>> proj_points;
  for (size_t i = 0; i < pc->size(); ++i)
  {
    Eigen::Vector3f rel_vec(pc->points[i].x - centroid[0], 
                            pc->points[i].y - centroid[1], 
                            pc->points[i].z - centroid[2]);
    float u = rel_vec.dot(axis_U);
    float v_val = rel_vec.dot(axis_V);
    proj_points.emplace_back(atan2(v_val, u), i);
  }

  std::sort(proj_points.begin(), proj_points.end());

  pcl::PointCloud<pcl::PointXYZ>::Ptr tmp_v(new pcl::PointCloud<pcl::PointXYZ>());
  tmp_v->resize(4);
  for (int i = 0; i < 4; ++i)
  {
    (*tmp_v)[i] = pc->points[proj_points[i].second];
  }
  ...
  for (int i = 0; i < 4; ++i)
  {
    Eigen::Vector3f rel_vec(tmp_v->points[i].x - centroid[0], tmp_v->points[i].y - centroid[1], tmp_v->points[i].z - centroid[2]);
    float u = rel_vec.dot(axis_U);
    if (u < min_u) { min_u = u; min_u_idx = i; }
  }

  v->resize(4);
  for (int i = 0; i < 4; ++i)
  {
    (*v)[i] = tmp_v->points[(min_u_idx + i) % 4];
  }
} 
